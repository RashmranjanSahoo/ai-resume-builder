...
const enhanceBullet = async (bullet, context) => {
    const prompt = `You are an expert resume writer. Enhance the following resume bullet point for a project called "${context.title || "a project"}" built with ${context.techStack?.join(", ") || "various technologies"}.
...
const ProjectsForm = ({ data = [], onChange }) => {
    ...
    const removeBullet = (projectIndex, bulletIndex) => {
        ...
        updated[projectIndex].description = updated[projectIndex].description.filter(
            (_, i) => i !== bulletIndex
        );
        onChange(updated);
    };
    ...
    const handleEnhanceBullet = async (projectIndex, bulletIndex) => {
        ...
        try {
            ...
            updateBullet(projectIndex, bulletIndex, enhanced);
        } catch (err) {
            ...
            {data.map((project, projectIndex) => (
                ...
                >
                    ...
                    <input
                        ...
                        value={project.date}
                        onChange={(e) => updateProject(projectIndex, "date", e.target.value)}
                        className="w-full border rounded-lg p-2"
                    />
                            ...
                            {(Array.isArray(project.description)
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-center px-2 pb-2 pt-1 border-t border-gray-100">
                                        <button
                                            onClick={() => handleEnhanceBullet(projectIndex, bulletIndex)}
                                            disabled={isLoading(projectIndex, bulletIndex) || !point?.trim()}
                                        ...
                                        >
                                            ...
                                            {isLoading(projectIndex, bulletIndex) ? "Enhancing..." : "AI Enhance"}
                                        </button>
